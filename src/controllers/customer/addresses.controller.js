...
exports.createAddress = async (req, res) => {
  ...
  try {
    ...
    const result = await db.Address.findByPk(data.address_id, {
      include: ['created']
    })
    dataResponse(
      res,
      result,
      'Address has been successfully created',
      'No Address has been successfully created'
    )
  } catch (err) {
    errResponse(res, err)
  }
...
exports.getAllAddress = async (req, res) => {
  ...
  try {
    ...
    dataResponse(
      res,
      data,
      'All Address has been retrieved',
      'No Address has been retrieved'
    )
  } catch (err) {
    errResponse(res, err)
  }
...
exports.getAddress = async (req, res) => {
  ...
  try {
    const data = await db.Address.findOne({
      where: { user_id: id, address_id: addressID }
    })
    dataResponse(
      res,
      data,
  ...
  try {
    ...
    if (result == 1) {
      ...
      dataResponse(
        ...
        'No Address has been successfully updated'
      )
    } else {
...
exports.updateDefaultAddress = async (req, res) => {
  ...
  try {
    const defaultAdd = await db.Address.findOne({
      where: { user_id: userID, is_default: 1 }
    })

    await db.Address.update(
      { is_default: '0' },
      { where: { address_id: defaultAdd[0].address_id, is_default: '1' } }
    )
...
exports.deleteAddress = async (req, res) => {
  ...
  try {
    ...
    if (del) {
      ...
      const data = await db.Address.findByPk(id, {
        paranoid: false,
        include: ['deleted']
      })
      dataResponse(
        ...
        'No address has been successfully deleted.'
      )
    } else {
