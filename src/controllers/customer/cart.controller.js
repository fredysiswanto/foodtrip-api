...
exports.addToCart = async (req, res) => {
  ...
  if (!(checkDish === null)) {
    ...
    await db.CartDetail.update(
      {
        subtotal:
          parseFloat(checkDish.subtotal) + parseFloat(dishDetails.dish_price)
      },
    ...
    dataResponse(
      res,
      data,
      'Dish has been successfully added to Cart',
      'No Dish has been successfully added to Cart'
    )
  }
}
...
exports.getCart = async (req, res) => {
  ...
  try {
    const data = await db.Cart.findOne({
      where: { created_by: userID },
      include: [
        { model: db.Dish, as: 'cart_dishes', include: ['dish_category'] },
        'restaurant'
      ]
    })
    if (data == null) emptyDataResponse(res, 'Cart is empty')
      ...
      dataResponse(
        res,
        data,
        'Cart has been successfully retrieved',
        'No Cart has been successfully retrieved'
      )
  } catch (err) {
    errResponse(res, err)
  }
...
exports.addQuantity = async (req, res) => {
  ...
  try {
    ...
    await db.CartDetail.update(
      {
        subtotal:
          parseFloat(cartDetails.subtotal) + parseFloat(dishDetails.dish_price)
      },
    ...
    dataResponse(
      res,
      data,
      'Added Dish Quantity Successfully',
      'Added Dish Quantity Failed'
    )
  } catch (err) {
    errResponse(res, err)
  }
  ...
  if (cartDetails.quantity == 1)
    ...
    try {
      ...
      await db.CartDetail.update(
        {
          subtotal:
            ...
            parseFloat(dishDetails.dish_price)
        },
        { where: { cartdetail_id: cartDetailsID } }
      )
      ...
      dataResponse(
        res,
        data,
        'Subtracted Dish Quantity Successfully',
        'Subtracted Dish Quantity Failed'
      )
    } catch (err) {
      errResponse(res, err)
    }
  ...
  await db.CartDetail.update(
    {
      quantity:
        sign === 'add'
          ? parseFloat(data.quantity) + parseFloat(val)
          : parseFloat(data.quantity) - parseFloat(val)
    },
