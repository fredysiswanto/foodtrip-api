...
async function checkApi({ url, method, headers = {}, query_params = {}, payload = {}, validate = {} }) {
  try {
    ...
    const validationResults = validateResponse(responseData, response.status, validate);
...
function validateResponse(response, status, validateRules) {
  let result = { status: "passed", errors: [] };

  // Validasi status HTTP
  if (validateRules.status && validateRules.status !== status) {
    result.status = "failed";
    result.errors.push(`Expected status ${validateRules.status}, but got ${status}`);
  }
    ...
    for (const key in validateRules.fields) {
      ...
      if (actualValue === undefined) {
        ...
        if (typeof actualValue !== expected) {
          result.status = "failed";
          result.errors.push(`Field ${key} expected type "${expected}", but got "${typeof actualValue}"`);
        }
      } else if (typeof expected === "object" && expected.type === "equal") {
        // ✅ Validasi Equal
        if (actualValue !== expected.value) {
          result.status = "failed";
          result.errors.push(`Field ${key} expected value "${expected.value}", but got "${actualValue}"`);
        }
      }
    }
  }
